...
let assert_can_add_purpose ~__context ~network ~current porpoise =
  let sop (*string-of-porpoise*) = Record_util.network_purpose_to_string in
  let reject str =
    raise Api_errors.(Server_error (network_incompatible_purposes, [ (sop porpoise); str ]))
  in
  let assert_no_net_has_bad_porpoise bads =
    ...
    List.iter (fun nwk ->
      ...
      List.iter (fun suspect -> if (List.mem suspect bads) then
        info "Cannot set new network purpose %s when there is a network with purpose %s" (sop porpoise) (sop suspect);
        reject (sop suspect)
      )
    )
  in
