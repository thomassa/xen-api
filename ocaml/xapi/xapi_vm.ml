...
let set_client_to_guest ~__context ~self ~value =
	Xapi_xenops.set_client_to_guest ~__context ~self value

let set_guest_to_client ~__context ~self ~value =
	Xapi_xenops.set_guest_to_client ~__context ~self value

let add_to_client_to_guest ~__context ~self ~key ~value =
	Db.VM.add_to_client_to_guest ~__context ~self ~key ~value;
	Xapi_xenops.set_client_to_guest ~__context ~self (Db.VM.get_client_to_guest ~__context ~self)

let remove_from_client_to_guest ~__context ~self ~key =
	Db.VM.remove_from_client_to_guest ~__context ~self ~key;
	Xapi_xenops.set_client_to_guest ~__context ~self (Db.VM.get_client_to_guest ~__context ~self)
...
(*		~guest_to_client *)
	...
	in
	Xapi_vm_helpers.create ~__context
		~name_label
		~name_description
		~user_version
		~is_a_template
		...
		~hardware_platform_version
		~client_to_guest
		~guest_to_client:[] (* MUSTELID!!!!! Should this be either removed or made a real value, or not? *)
