  ...
  then Some(Api_errors.other_operation_in_progress,["VDI"; _ref])
    ...
    then Some(Api_errors.sr_no_pbds, [Ref.string_of sr])
      ...
      let allowed_for_cbt_metadata_vdi = match op with
        | `clone | `copy | `mirror | `resize | `resize_online | `snapshot | `set_on_boot -> false
        | `blocked | `destroy | `disable_cbt | `enable_cbt | `force_unlock | `forget | `generate_config | `scan | `update -> true in
...
let set_on_boot ~__context ~self ~value =
  ...
  transform_storage_exn
    (fun () ->
       C.VDI.set_persistent ~dbg:(Ref.string_of task) ~sr ~vdi ~persistent:(value = `persist);
    );
